...
def train(inn_var, BCs, ICs, out_true, model, Loss, optimizer, scheduler, log_loss, opts):
    ...
    def closure():
        ...
        loss_batch.backward()
        ve_loss = Loss(model.get_params()[0], torch.tensor(0.001, dtype=torch.float32).cuda())  # 测试inverse
        K_loss = Loss(model.get_params()[0], torch.tensor(0.001, dtype=torch.float32).cuda())  # 测试inverse
        log_loss.append([ve_loss.item(), K_loss.item(), eqs_loss.item(), geqs_loss.item(), bcs_loss_1.item(), data_loss.item(), ])
        return loss_batch
...
if __name__ == '__main__':
    ...
    for N in Ns:
        opts.Nx_EQs = N
        work_name = 'brinkman_forchheimer_2-' + opts.net_type + '-N_' + str(opts.Nx_EQs) + opts.work_name
        ...
        for iter in range(start_epoch, opts.epochs_adam):
            ...
            if iter < opts.epochs_adam:
                ...
                log_par.append(torch.tensor(Net_model.get_params()).detach().cpu().numpy())
            else:
            ...
            L2_ve = np.mean(np.linalg.norm(log_par[-1][0] - 0.001) / np.linalg.norm(0.001))
            L2_K = np.mean(np.linalg.norm(log_par[-1][1] - 0.001) / np.linalg.norm(0.001))
            log_L2.append([L2_ve, L2_K, L2_u])

            if iter > 0 and iter % opts.print_freq == 0:
                print('iter: {:6d}, lr: {:.1e}, cost: {:.2f}, pred_ve: {:.2e} , true_ve: 1e-3, pred_K: {:.2e} , true_K: 1e-3\n'
                      've_loss: {:.2e}, K_loss: {:.2e}, EQs_loss: {:.2e}, GEQs_loss: {:.2e}, BCs_loss: {:.2e}, DTs_loss_0: {:.2e}'.
                      format(iter, learning_rate, time.time() - star_time, log_par[-1][0], log_par[-1][1],
                ...
                plt.clf()
                plt.subplot(2, 1, 1)
                Visual.plot_loss(np.arange(len(log_loss)), np.array(log_loss)[:, 0], 've_loss')
                Visual.plot_loss(np.arange(len(log_loss)), np.array(log_loss)[:, 1], 'K_loss')
                Visual.plot_loss(np.arange(len(log_loss)), np.array(log_loss)[:, 2], 'eqs_loss')
                Visual.plot_loss(np.arange(len(log_loss)), np.array(log_loss)[:, 3], 'geqs_loss')
                plt.subplot(2, 1, 2)
                Visual.plot_loss(np.arange(len(log_loss)), np.array(log_loss)[:, 4], 'BCS_loss')
                Visual.plot_loss(np.arange(len(log_loss)), np.array(log_loss)[:, 5], 'DTS_loss')
                plt.savefig(os.path.join(tran_path, 'log_loss.svg'))

                plt.figure(5, figsize=(20, 15))
                plt.clf()
                Visual.plot_loss(np.arange(len(log_par)), np.array(log_par)[:, 0], 've of GPINN')
                Visual.plot_loss(np.arange(len(log_par)), np.array(log_par)[:, 1], 'K of GPINN')
                Visual.plot_loss(np.arange(len(log_par)), np.ones(len(log_par)) * 0.001, 'EXACT')
                plt.xlabel("Epoch")
                plt.ylabel("v_e")
                plt.savefig(os.path.join(tran_path, 'log_ve_K.svg'))
                ...
                plt.clf()
                Visual.plot_loss(np.arange(len(log_L2)), np.array(log_L2)[:, 0], 've')
                Visual.plot_loss(np.arange(len(log_L2)), np.array(log_L2)[:, 1], 'K')
                plt.ylabel("L2_ve")
                plt.savefig(os.path.join(tran_path, 'L2_ve_K.svg'))
